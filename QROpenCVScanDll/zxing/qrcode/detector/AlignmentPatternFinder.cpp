...
/*
Added by JSmith 20:00 on the 22/6/20 to try and fix compilation issues on nix. Does not like nan and isnan. Remove the following 2 lines and this comment to return to normal


#define nan(a) std::nan(a)
#define isnan(a) std::isnan(a)

OK the above defines did not work so I did a control f through the document and manually replaced all calls to nan and isnan with std::nan and std::isnan

That did not work so now have pull a null pointer to all nan() calls and changed isnan call to std::isnan.

That worked but it turns out many files in this project have calls to nan and isnan which are all failing. Very hacky to change them all. Lets move on to other options need to fix this globally somehow.

Let's try adding using namespace std to the start and compiling. OK this fixed the isnans but not the nan's

Let's try putting a macro in that defines the nan() function as 
*/
...
//included this to get the nan's to work with g++
